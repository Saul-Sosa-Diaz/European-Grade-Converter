...
export class IrelandGradeConverter implements ICountryConverter {
  ...
  convertToDestinationCountry(grade: number): string {
    ...
    const RESULT = (RANGE.min + (grade - RANGE.base) * (RANGE.max + 1 - RANGE.min)).toFixed(2)
    return ` ${RESULT}% (${RANGE.name})`
  }
  convertToSpain(grade: string): string {
    const NewGrade = parseFloat(grade)
    if (NewGrade === this.gradeRanges[this.gradeRanges.length - 1].max + 1) return '10' // if the grade is 100, return 10
    const RANGE = this.gradeRanges.find((r) => NewGrade >= r.min && NewGrade <= r.max) // Find the range of the grade
    if (!RANGE) return '0' // if the range is not found, return 0
    const result = RANGE.base + (NewGrade - RANGE.min) / (RANGE.max + 1 - RANGE.min)
